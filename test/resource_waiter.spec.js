...
describe('AWS.ResourceWaiter', function() {
  beforeEach(function() {
    this.skip('ResourceWaiter tests need to be updated to use S3 services.');
  });

  return describe('wait', function() {
    it('waits until a given state is met', function() {
      var data, s3, err, resp, waiter;
      err = null;
      data = null;
      resp = null;
      s3 = new AWS.S3();
    ...
    it('supports path matcher', function() {
      ...
      waiter = new AWS.ResourceWaiter(s3, 'distributionDeployed');
      waiter.wait(function(e, d) {
        resp = this;
        err = e;
        return data = d;
    ...
    it('supports custom delay and maxAttempts', function() {
      ...
      helpers.mockResponses(resps);
      waiter = new AWS.ResourceWaiter(s3, 'tableExists');
      waiter.wait({
        $waiter: {
          delay: delay,
