...
AWS.Credentials = AWS.util.inherit({
  ...
  needsRefresh: function needsRefresh() {
    ...
    if (this.tokenManager) {
        return !this.tokenManager.token || this.tokenManager.token.isExpired();
  ...
  refresh: function refresh(callback) {
    if (this.tokenManager && this.tokenManager.refreshToken) {
        // This should generally only happen if either a token refresh failed
        // or someone tried to use a token before we were able to retrieve it.
        this.tokenManager.refreshToken().then(function() {
            callback();
        }).catch(function(err) {
            callback(err);
        });
