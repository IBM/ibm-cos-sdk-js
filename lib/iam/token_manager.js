        ...
        if (typeof config.authCallback === 'function') {
            ...
            if (typeof config.authCallback.then === 'function') {
                ...
                this._tokenRequest = new Promise(function(resolve, reject) {
                    try {
                        var res = config.authCallback(function(tokenConfig) {
                            resolve(this.createToken(tokenConfig));
                        }.bind(this));
                        if (res && typeof res.then === 'function') {
                            resolve(res.then(function(tokenConfig) {
                                return this.createToken(tokenConfig);
                            }.bind(this)));
                        }
                    } catch (err) {
                        reject(err);
