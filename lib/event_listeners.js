...
AWS.EventListeners = {
  Core: new SequentialExecutor().addNamedListeners(function(add, addAsync) {
    ...
    addAsync('COMPUTE_CONTENT_MD5', 'afterBuild', function COMPUTE_CONTENT_MD5(req, done) {
      req.haltHandlersOnError();
      if (req.service.getSignerClass(req) === AWS.Signers.IAM) {
        var body = req.httpRequest.body || '';
        AWS.util.computeContentMD5(body, function(err, md5) {
          if (err) {
            done(err);
          }
          else {
